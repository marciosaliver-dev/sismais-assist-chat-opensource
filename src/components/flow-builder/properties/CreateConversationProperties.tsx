...
export function CreateConversationProperties({ config, onUpdate, onBatchUpdate }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <WhatsAppInstanceSelect
        ...
        label="Canal para iniciar contato"
        required
      />
      <label className="flex items-center gap-2 text-xs">
        <Checkbox checked={!!config.link_same_client} onCheckedChange={v => onUpdate('link_same_client', !!v)} />
        Vincular ao mesmo cliente
      </label>
    </div>
  )
}
