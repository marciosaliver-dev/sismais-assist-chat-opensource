...
export function MoveToBoardProperties({ config, onUpdate, onBatchUpdate }: Props) {
  ...
  const { data: stages = [] } = useQuery({
    ...
    queryFn: async () => { const { data } = await (supabase as any).from('kanban_stages').select('id, name, is_entry').eq('board_id', config.board_id).eq('active', true).order('sort_order'); return data || [] },
  })

  return (
    <div className="space-y-3">
      <div><Label className="text-xs">Board Destino</Label>
      ...
      {config.board_id && (
        <div><Label className="text-xs">Etapa de Entrada</Label>
          <Select value={config.stage_id || ''} onValueChange={v => { const s = stages.find((s: any) => s.id === v); onBatchUpdate({ stage_id: v, stage_name: s?.name || '' }) }}>
            <SelectTrigger className="text-xs mt-1"><SelectValue placeholder="Selecionar..." /></SelectTrigger>
            <SelectContent>{stages.map((s: any) => <SelectItem key={s.id} value={s.id}>{s.name}{s.is_entry ? ' (entrada)' : ''}</SelectItem>)}</SelectContent>
          </Select>
        </div>
      )}
    </div>
