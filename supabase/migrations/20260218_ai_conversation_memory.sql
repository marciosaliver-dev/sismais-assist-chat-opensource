-- Migration: ai_conversation_memory
-- Creates conversation memory tables for persistent context across agent interactions
...
CREATE POLICY "Service role full access ai_conversation_memory" ON public.ai_conversation_memory FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Authenticated read ai_conversation_memory" ON public.ai_conversation_memory FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "Service role full access ai_customer_memory" ON public.ai_customer_memory FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Authenticated read ai_customer_memory" ON public.ai_customer_memory FOR SELECT TO authenticated USING (true);

COMMENT ON TABLE public.ai_customer_memory IS 'Memoria de longo prazo sobre clientes para personalizacao do atendimento.';

-- 3. Session context table (for active conversations)
...
CREATE POLICY "Service role full access ai_session_context" ON public.ai_session_context FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Authenticated read ai_session_context" ON public.ai_session_context FOR SELECT TO authenticated USING (true);

COMMENT ON TABLE public.ai_session_context IS 'Contexto de sessao para rastreamento de estado em conversacoes ativas.';

-- 4. Agent knowledge graph (relationships between entities)
...
CREATE POLICY "Service role full access ai_knowledge_graph" ON public.ai_knowledge_graph FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Authenticated read ai_knowledge_graph" ON public.ai_knowledge_graph FOR SELECT TO authenticated USING (true);

COMMENT ON TABLE public.ai_knowledge_graph IS 'Grafo de conhecimento para representar relacoes entre entidades do sistema.';

-- 5. Function to store conversation memory
...
-- 6. Function to retrieve conversation memory
...
-- 7. Function to store customer memory
...
-- 8. Function to get customer memory
...
-- 9. Function to generate conversation summary
...
BEGIN
  ...
  IF array_length(v_intents, 1) > 0 THEN
    v_summary := v_summary || 'Intenções identificadas: ' || array_to_string(v_intents, ', ') || '. ';
  END IF;
...
-- 10. Function to update session context
...
-- 11. View: Combined conversation context
...
VALUES ('20260218_ai_conversation_memory', 'AI conversation memory system - context persistence across interactions', NOW())
