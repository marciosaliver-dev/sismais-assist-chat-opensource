...
SELECT cron.schedule(
  ...
  $$
    ...
    SELECT 
      ...
      c.id,
      'escalate-to-human',
      json_build_object('reason', 'Ticket sem atividade ha mais de 7 dias'),
...
SELECT cron.schedule(
  ...
  $$
    ...
    SELECT 
      ...
      'send-message',
      json_build_object(
        'message', 'Ola! Notamos que voce ficou um tempo sem responder. Posso ajudar em algo?',
...
-- 7. Daily Reconciliation (at midnight)
SELECT cron.schedule(
...
-- 8. Health Check (every 30 minutes)
SELECT cron.schedule(
  'health-check',
  '*/30 * * * *',
  $$
...
-- 9. Unassigned Queue Check (every 10 minutes)
SELECT cron.schedule(
  ...
  $$
    ...
    SELECT 
      ...
      c.id,
      'escalate-to-human',
      json_build_object('reason', 'Ticket na fila ha mais de 30 minutos'),
      json_build_object('escalated_at', NOW()),
      true
    FROM ai_conversations c
    WHERE c.status = 'novo'
...
SELECT 'Migration 1: pg_cron_setup completed' as status;
